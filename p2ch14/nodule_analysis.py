...
def print_confusion(label, confusions, do_mal):
    ...
    for i, (l, r) in enumerate(zip(row_labels, confusions)):  # noqa: E741
        r = [l] + list(r)  # noqa: RUF005
        if i == 0:
            r[1] = ""
        print(" | ".join([f.format(i) for i in r]))


def match_and_score(detections, truth, threshold=0.5, threshold_mal=0.5):  # noqa: ARG001
    ...
    confusion = np.zeros((3, 4), dtype=np.int)  # noqa: NPY001
    if len(detected_xyz) == 0:
        ...
        matched_true_nodules = np.zeros(len(true_nodules), dtype=np.int)  # noqa: NPY001
...
class NoduleAnalysisApp:
    ...
    def main(self):
        ...
        all_confusion = np.zeros((3, 4), dtype=np.int)  # noqa: NPY001
        for _, series_uid in series_iter:
            ...
            if not self.cli_args.run_validation:
                ...
                for prob, prob_mal, center_xyz, center_irc in classifications_list:  # noqa: B007
                    if prob > 0.5:
    ...
    def classifyCandidates(self, ct, candidateInfo_list):
        ...
        for batch_ndx, batch_tup in enumerate(cls_dl):  # noqa: B007
